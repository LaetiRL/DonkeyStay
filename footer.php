</main>
<footer class="container pt-4 my-md-5 pt-md-5">
    <div class="row">
        <div class="col-6 col-md footer-d-flex">
            <h4 class="h6 mb-4">Options et Assurance</h4>
            <ul class="list-unstyled text-small">
                <li><a class="text-muted" href="terms_animaux.php">Option Animaux</a></li>
                <li><a class="text-muted" href="annulation.php">Assurance annulation</a></li>
            </ul>
        </div>
        <div class="col-6 col-md footer-d-flex">
            <h4 class="h6 mb-4">Termes et Conditions</h4>
            <ul class="list-unstyled text-small">
                <li><a class="text-muted" href="confidentialite.php">Politique de Confidentialité</a></li>
                <li><a class="text-muted" href="terms.php">Conditions Générales</a></li>
            </ul>
        </div>
    </div>
    <div class="footer">DonkeyStay.com - Copyright © 2022 - All rights reserved.</div>
</footer>
<!-- search bar autocomplete -->
<script type="text/javascript">
    $(function() {
        $("#request").autocomplete({
            source: 'ajax-db-search.php',
        });
    });
</script>
</body>

</html>